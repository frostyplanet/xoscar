...
class ActorCaller:
    ...
    async def call_with_client(
        self, client: CallerClient | Client, message: _MessageBase, wait: bool = True
        ...
        """
        if not isinstance(client, CallerClient):
            # Compatible code that might be deprecated
            caller_client = self._clients.get(client, None)
            if caller_client is None:
                assert client.dest_address is not None
                caller_client = self._add_client(
                    client.dest_address, client.__class__, client
                )
        else:
        ...
        with Timer() as timer:
            await caller_client.send(message, wait_response)
    ...
    async def call_send_buffers(
        ...
        if not isinstance(client, CallerClient):
            ...
            assert isinstance(client._inner, UCXClient)
            caller_client = client

        loop = asyncio.get_running_loop()
        wait_response = loop.create_future()
        with Timer() as timer:
            await caller_client.send(
                meta_message, wait_response, local_buffers=local_buffers
            )
            # NOTE: When send raise exception, we should not _force_remove_client,
            # let _listen_task exit normally on client close,
            # and set all futures in client to exception
